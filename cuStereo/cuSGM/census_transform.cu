...
static constexpr int WINDOW_WIDTH  = 9;//编译时常量
...
{
	using pixel_type = T;
	static const int SMEM_BUFFER_SIZE = WINDOW_HEIGHT + 1;//8

	const int half_kw = WINDOW_WIDTH  / 2;//4
	const int half_kh = WINDOW_HEIGHT / 2;//3

	__shared__ pixel_type smem_lines[SMEM_BUFFER_SIZE][BLOCK_SIZE];//共享内存：大小 = 每个block的线程数据 * （窗口高度 + 1）

	const int tid = threadIdx.x;//当前线程
	const int x0 = blockIdx.x * (BLOCK_SIZE - WINDOW_WIDTH + 1) - half_kw;//blockIdx.x * 
	const int y0 = blockIdx.y * LINES_PER_BLOCK;//  blockIdx.y * 16
